        ...
        <div class="card">
            <div class="card-content">
                <div class="card-body" id="form">
                    ...
                    <?php echo Form::open(['method' => 'POST', 'route' => 'admin.'.request()->segment(2).'.store', 'class' => 'form-horizontal','id'=>'vendorForm']); ?>

                    
                        <div class="form-group<?php echo e($errors->has('name') ? ' has-error' : ''); ?>">
                            <?php echo Form::label('name', 'Vendor Name'); ?>

                            <?php echo Form::text('name', null, ['class' => 'form-control slugify', 'required' => 'required','placeholder'=>'Enter Vendor Name']); ?>

                            <small class="text-danger"><?php echo e($errors->first('name')); ?></small>
                        </div>
                    
                        <div class="btn-group">
                            <?php echo Form::button("Save Vendor", ['class' => 'btn btn-success btn-border','onClick'=>'createVendor(this)']); ?>
                    ...
                    <table id="dataTableAjax" class="display dataTableAjax table table-striped table-bordered dom-jQuery-events" >
                        <thead>
                            <tr>
                                ...
                                <th>Slug</th>
                                <th>Created at</th>
                                <?php if (\Illuminate\Support\Facades\Blade::check('can', ['edit_vendor','delete_vendor'])): ?>
        ...
        { "data": "created_at" }, 
        {
            ...
            render: function(data, type, row) {
                if (type === 'display') {
                    ...
                    <?php if (\Illuminate\Support\Facades\Blade::check('can', ['edit_vendor','delete_vendor','read_vendor'])): ?>

                    <?php if (\Illuminate\Support\Facades\Blade::check('can', 'read_vendor')): ?>
                    ...
                    <?php if (\Illuminate\Support\Facades\Blade::check('can', 'edit_vendor')): ?>
                    ...
                    <?php if (\Illuminate\Support\Facades\Blade::check('can', 'delete_vendor')): ?>
...
var createForm = '<form method="POST" action="<?php echo e(route('admin.'.request()->segment(2).'.index')); ?>" accept-charset="UTF-8" class="form-horizontal" id="vendorForm"><?php echo e(csrf_field()); ?><div class="form-group"><label for="name">Vendor Name</label><input class="form-control" required="required" placeholder="Enter Vendor Name" name="name" type="text" id="name"><small class="text-danger"></small></div><div class="btn-group"><button class="btn btn-success btn-border" onclick="createVendor(this)" type="button">Save Vendor</button></div></form>';
...
function editData(url) {
    ...
    $.ajax({
        ...
        url:url+'/edit',
        success:function(response){
           $('#form').html('<form id="vendorForm" method="POST" action="<?php echo e(route('admin.'.request()->segment(2).'.index')); ?>" accept-charset="UTF-8"><?php echo e(method_field('PUT')); ?> <?php echo e(csrf_field()); ?><div class="form-group"><label for="name">Vendor Name</label><input class="form-control" required="required" value="'+response.data.name+'" placeholder="Enter Vendor Name" name="name" type="text" id="name"><small class="text-danger"></small></div><div class="btn-group"><button class="btn btn-success btn-border" onclick="UpdateVendor(this,'+response.data.id+')" type="button">Update Vendor</button></div></form>');
        },
        error:function(error){
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('admin.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/asifjamal/Documents/ecommerce/resources/views/admin/vendor/list.blade.php ENDPATH**/ ?>